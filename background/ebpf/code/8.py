...
class Pkt5Tuple(ctypes.Structure):
    ...
    def __str__(self):
        ...
        return str
def get_cycles():
    cmd = "sudo bpftool prog profile name xdp_prog1 duration 20 cycles"
    ...
    for i in str_1:
        if i!='' and i[0]>='0' and i[0]<='9':
            num.append(int(i))
    outputfn=f'cycles/exp-cycles-8.log'
...
if __name__ == '__main__':
    ...
    parser.add_argument("-r", "--read", type=int, default=30,help="Read throughput after X time and print result")
    ...
    # load BPF program
    b = BPF(src_file=f'{sys.path[0]}/ebpf/new/bloomfilter/es.h', cflags=custom_cflags, device=None)
