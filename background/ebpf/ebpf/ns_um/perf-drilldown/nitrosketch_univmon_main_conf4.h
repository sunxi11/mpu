...
static void __always_inline ns_um_add_with_hash(struct countsketch *cs, uint16_t *hashes, uint32_t row_to_update)
{
		...
		if (CHECK_BIT(hash, 15)) {
            ...
            // __sync_fetch_and_add(&cs->values[row_to_update][target_idx], 1);
		} else {
            cs->values[row_to_update][target_idx]--;
            // __sync_fetch_and_sub(&cs->values[row_to_update][target_idx], 1);
		}
}
...
int xdp_prog1(struct CTXTYPE *ctx) {
    ...
    uint32_t layerhash = fasthash32(&pkt, sizeof(pkt), 0xdeadbeef);
    ...
    uint64_t h = fasthash64(&pkt, sizeof(pkt), 0x2d31e867);
