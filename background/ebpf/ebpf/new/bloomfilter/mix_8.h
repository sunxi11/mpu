...
BPF_PERCPU_ARRAY(countsketch71, __u32, 26042);
BPF_PERCPU_ARRAY(countsketch72, __u32, 26042);

BPF_ARRAY(countsketch81, __u32, 327680);
BPF_ARRAY(countsketch82, __u32, 327680);
BPF_ARRAY(countsketch83, __u32, 327680);
BPF_ARRAY(countsketch84, __u32, 327680);
BPF_ARRAY(countsketch85, __u32, 327680);
BPF_ARRAY(countsketch86, __u32, 327680);
BPF_ARRAY(countsketch87, __u32, 327680);
BPF_ARRAY(countsketch88, __u32, 327680);
...
#pragma unroll
  ...
  __u32 hash21 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 0x12345678);
    ...
    for (unsigned i = 0; i < ct_k; i++)
    {
       ...
       uint32_t zero6 = 0;
    bool update_cm6 = false;
    __u64 hpp6 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 0x1234) % 4;
    if (hpp6 <= 1)
        update_cm6 = false; // p(goto fastpath6) = 0.4
    else
        update_cm6 = true; // p(goto normal path) = 0.6
    ...
    if (update_cm6)
    {
        ...
        __u32 target_idx61 = hash61 & (COLUMNS6 - 1);
        __u32 target_idx62 = hash62 & (COLUMNS6 - 1);
        __u32 target_idx63 = hash63 & (COLUMNS6 - 1);
        __u32 target_idx64 = hash64 & (COLUMNS6 - 1);
        __u32 target_idx65 = hash65 & (COLUMNS6 - 1);
    ...
    {
        __u64 *zero6 = 0, *val6;
        val6 = fastpath6.lookup_or_try_init(&pkt, &zero6);
        if (val6)
        {
            (*val6) += 1;
        }
...
//    struct pkt_5tuple pkt5;
    ...
    __u32 target_idx71 = hash71 & (COLUMNS7 - 1);
    ...
    __u32 target_idx72 = hash72 & (COLUMNS7 - 1);
    // We should probably split the coin here to swap the sign for the countsketch7 implementation
    // 小于2^n的，CHECK_BIT(hash7, n)为0，2*2^n ~ 3*2^n CHECK_BIT(hash7, n)为0
    ...
    u32 rand81 = bpf_get_prandom_u32();
    if (rand81 < UPDATE_PROBABILITY)
    {
        // Here we start updating the sketch
        __u32 hash81 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 1);
        __u32 target_idx81 = hash81 & (COLUMNS - 1);
        // We should probably split the coin here to swap the sign for the countsketch8 implementation
        // 小于2^n的，CHECK_BIT(hash8, n)为0，2*2^n ~ 3*2^n CHECK_BIT(hash8, n)为0
        __u32 *val81 = countsketch81.lookup(&target_idx81);
        if (val81)
        {
            if (CHECK_BIT(hash81, 31))
            {
    ...
    u32 rand82 = bpf_get_prandom_u32();
    if (rand82 < UPDATE_PROBABILITY)
    {
        // Here we start updating the sketch
        __u32 hash82 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 2);
    ...
    u32 rand83 = bpf_get_prandom_u32();
    if (rand83 < UPDATE_PROBABILITY)
    {
        // Here we start updating the sketch
        __u32 hash83 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 3);
    ...
    u32 rand84 = bpf_get_prandom_u32();
    if (rand84 < UPDATE_PROBABILITY)
    {
        // Here we start updating the sketch
        __u32 hash84 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 4);
    ...
    u32 rand85 = bpf_get_prandom_u32();
    if (rand85 < UPDATE_PROBABILITY)
    {
        // Here we start updating the sketch
        __u32 hash85 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 5);
    ...
    u32 rand86 = bpf_get_prandom_u32();
    if (rand86 < UPDATE_PROBABILITY)
    {
        // Here we start updating the sketch
        __u32 hash86 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 6);
        __u32 target_idx86 = hash86 & (COLUMNS - 1);
        // We should probably split the coin here to swap the sign for the countsketch8 implementation
        // 小于2^n的，CHECK_BIT(hash8, n)为0，2*2^n ~ 3*2^n CHECK_BIT(hash8, n)为0
        __u32 *val86 = countsketch86.lookup(&target_idx86);
        if (val86)
        {
            if (CHECK_BIT(hash86, 31))
            {
    ...
    u32 rand87 = bpf_get_prandom_u32();
    if (rand87 < UPDATE_PROBABILITY)
    {
        // Here we start updating the sketch
        __u32 hash87 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 7);
        __u32 target_idx87 = hash87 & (COLUMNS - 1);
        // We should probably split the coin here to swap the sign for the countsketch8 implementation
        // 小于2^n的，CHECK_BIT(hash8, n)为0，2*2^n ~ 3*2^n CHECK_BIT(hash8, n)为0
        __u32 *val87 = countsketch87.lookup(&target_idx87);
        if (val87)
        {
            if (CHECK_BIT(hash87, 31))
            {
     ...
     u32 rand88 = bpf_get_prandom_u32();
    if (rand88 < UPDATE_PROBABILITY)
    {
        // Here we start updating the sketch
        __u32 hash88 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 8);
        __u32 target_idx88 = hash88 & (COLUMNS - 1);
        // We should probably split the coin here to swap the sign for the countsketch8 implementation
        // 小于2^n的，CHECK_BIT(hash8, n)为0，2*2^n ~ 3*2^n CHECK_BIT(hash8, n)为0
        __u32 *val88 = countsketch88.lookup(&target_idx88);
        if (val88)
        {
            if (CHECK_BIT(hash88, 31))
            {
