...
simple_fwd_process_pkts_pipe(void *process_pkts_params)
{
	...
	int operate;
	uint64_t cur_tsc, last_tsc;
	uint64_t step1 = 0, step2 = 0, step3 = 0;
	struct rte_mbuf *mbufs[VNF_RX_BURST_SIZE];
	uint16_t j, nb_rx, queue_id;
	uint32_t port_id = 0, core_id = rte_lcore_id();
	struct vnf_per_core_params *params = &core_params_arr[core_id];
	...
	if(core_id == 1 || core_id == 2 || core_id == 3 || core_id == 4)
		operate = 1;
			...
			while (!force_quit) {
				if(core_id == 0){
					if(step1 > 10000000){
						...
						printf("step1: avg_cycles:%d\n", (cycles_end - cycles_start) / step1);
		...
		case 1:{
			...
			while(!force_quit){
				if(core_id == 1){
					if(step2 > 10000000){
						uint64_t cycles_end = rte_rdtsc();
						printf("step2: avg_cycles:%d\n", (cycles_end - cycles_start) / step2);
						cycles_start = cycles_end;
						step2 = 0;
					}
				}
		...
		case 2:{
			...
			while(!force_quit){
				if(core_id == 5){
					if(step3 > 10000000){
						uint64_t cycles_end = rte_rdtsc();
						printf("step3: avg_cycles:%d\n", (cycles_end - cycles_start) / step3);
						cycles_start = cycles_end;
						step3 = 0;
					}
				}
