...
BPF_ARRAY(LightPart36, __u32, 245760);
BPF_ARRAY(LightPart46, __u32, 245760);
BPF_ARRAY(LightPart56, __u32, 245760);
BPF_ARRAY(LightPart66, __u32, 245760);
BPF_ARRAY(LightPart76, __u32, 245760);
BPF_ARRAY(LightPart86, __u32, 245760);
BPF_ARRAY(HeavyParts_flag6, __u32, 100000);
BPF_ARRAY(HeavyParts_buckets6, struct Bucket, 100000);

BPF_ARRAY(LightPart17, __u32, 245760);
BPF_ARRAY(LightPart27, __u32, 245760);
BPF_ARRAY(LightPart37, __u32, 245760);
BPF_ARRAY(LightPart47, __u32, 245760);
BPF_ARRAY(LightPart57, __u32, 245760);
BPF_ARRAY(LightPart67, __u32, 245760);
BPF_ARRAY(LightPart77, __u32, 245760);
BPF_ARRAY(LightPart87, __u32, 245760);
BPF_ARRAY(HeavyParts_flag7, __u32, 100000);
BPF_ARRAY(HeavyParts_buckets7, struct Bucket, 100000);

BPF_ARRAY(LightPart18, __u32, 245760);
BPF_ARRAY(LightPart28, __u32, 245760);
BPF_ARRAY(LightPart38, __u32, 245760);
BPF_ARRAY(LightPart48, __u32, 245760);
BPF_ARRAY(LightPart58, __u32, 245760);
BPF_ARRAY(LightPart68, __u32, 245760);
BPF_ARRAY(LightPart78, __u32, 245760);
BPF_ARRAY(LightPart88, __u32, 245760);
BPF_ARRAY(HeavyParts_flag8, __u32, 100000);
BPF_ARRAY(HeavyParts_buckets8, struct Bucket, 100000);
...
#pragma unroll
   ...
   uint32_t hash8 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 1234);
    __u32 pos8 = hash8 & (heavy_size - 1);

    struct Bucket *hpb8;
    struct Bucket *hpb8_init;
    // hpb8_init->posvote = 1;
    // hpb8_init->negvote = 0;
    // hpb8_init->flag == false;
    // hpb8_init->flow_id.dstIP = dstIP;
    // hpb8_init->flow_id.srcIP = srcIP;
    __u32 *hpf8;
    hpf8 = HeavyParts_flag8.lookup(&pos8);
    hpb8 = HeavyParts_buckets8.lookup(&pos8);
    ...
    if (hpf8)
    {
        if (hpb8)
        {
            if (*hpf8 == 0)
            {
                // hpf8 = HeavyParts_flag8.lookup_or_try_init(&pos, &one);
                *hpf8 = 1;
                // hpb8 = HeavyParts_buckets8.lookup_or_try_init(&pos, &hpb8_init);
                hpb8->posvote = 1;
                hpb8->negvote = 0;
                hpb8->flag == false;
                hpb8->flow_id.dstIP = dstIP;
                hpb8->flow_id.srcIP = srcIP;
            }
            else if (*hpf8 != 0)
            {
                if (hpb8->flow_id.srcIP == srcIP && hpb8->flow_id.dstIP == dstIP)
                {
                    hpb8->posvote++;
                }
                else
                {
                    hpb8->negvote++;
                    u_int64_t temp8 = hpb8->negvote / hpb8->posvote;
                    if (temp8 < lamda)
                    {
                        res8 = 1;
                    }
                    else
                    {
                        ...
                        hpb8->posvote = 1;
                        uint32_t tmp8 = srcIP;
                        srcIP = hpb8->flow_id.srcIP;
                        hpb8->flow_id.srcIP = tmp8;
                        tmp8 = dstIP;
                        dstIP = hpb8->flow_id.dstIP;
                        hpb8->flow_id.dstIP = tmp8;
                    }
                }
            }
            ...
            if (res8 > 0)
            {
                ...
                uint32_t hash18 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 11);
                uint32_t hash28 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 22);
                uint32_t hash38 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 33);
                uint32_t hash48 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 44);
                uint32_t hash58 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 55);
                uint32_t hash68 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 66);
                uint32_t hash78 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 77);
                uint32_t hash88 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 88);
                __u32 pos18 = hash18 & (light_size_n - 1);
                __u32 pos28 = hash28 & (light_size_n - 1);
                __u32 pos38 = hash38 & (light_size_n - 1);
                __u32 pos48 = hash48 & (light_size_n - 1);
                __u32 pos58 = hash58 & (light_size_n - 1);
                __u32 pos68 = hash68 & (light_size_n - 1);
                __u32 pos78 = hash78 & (light_size_n - 1);
                __u32 pos88 = hash88 & (light_size_n - 1);
                ...
                __u32 *val38 = LightPart38.lookup(&hash38);
                if (val38)
                    (*val38) += 1;
                __u32 *val48 = LightPart48.lookup(&hash48);
                if (val48)
                    (*val48) += 1;
                __u32 *val58 = LightPart58.lookup(&hash58);
                if (val58)
                    (*val58) += 1;
                __u32 *val68 = LightPart68.lookup(&hash68);
                if (val68)
                    (*val68) += 1;
                __u32 *val78 = LightPart78.lookup(&hash78);
                if (val78)
                    (*val78) += 1;

                __u32 *val88 = LightPart88.lookup(&hash88);
                if (val88)
                    (*val88) += 1;
            }
    ...
    uint32_t hash7 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 1234);
    __u32 pos7 = hash7 & (heavy_size - 1);

    struct Bucket *hpb7;
    struct Bucket *hpb7_init;
    // hpb7_init->posvote = 1;
    // hpb7_init->negvote = 0;
    // hpb7_init->flag == false;
    // hpb7_init->flow_id.dstIP = dstIP;
    // hpb7_init->flow_id.srcIP = srcIP;
    __u32 *hpf7;
    hpf7 = HeavyParts_flag7.lookup(&pos7);
    hpb7 = HeavyParts_buckets7.lookup(&pos7);
    ...
    if (hpf7)
    {
        if (hpb7)
        {
            if (*hpf7 == 0)
            {
                // hpf7 = HeavyParts_flag7.lookup_or_try_init(&pos, &one);
                *hpf7 = 1;
                // hpb7 = HeavyParts_buckets7.lookup_or_try_init(&pos, &hpb7_init);
                hpb7->posvote = 1;
                hpb7->negvote = 0;
                hpb7->flag == false;
                hpb7->flow_id.dstIP = dstIP;
                hpb7->flow_id.srcIP = srcIP;
            }
            else if (*hpf7 != 0)
            {
                if (hpb7->flow_id.srcIP == srcIP && hpb7->flow_id.dstIP == dstIP)
                {
                    hpb7->posvote++;
                }
                else
                {
                    hpb7->negvote++;
                    u_int64_t temp7 = hpb7->negvote / hpb7->posvote;
                    if (temp7 < lamda)
                    {
                        res7 = 1;
                    }
                    else
                    {
                        ...
                        hpb7->posvote = 1;
                        uint32_t tmp7 = srcIP;
                        srcIP = hpb7->flow_id.srcIP;
                        hpb7->flow_id.srcIP = tmp7;
                        tmp7 = dstIP;
                        dstIP = hpb7->flow_id.dstIP;
                        hpb7->flow_id.dstIP = tmp7;
                    }
                }
            }
            ...
            if (res7 > 0)
            {
                ...
                uint32_t hash17 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 11);
                uint32_t hash27 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 22);
                uint32_t hash37 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 33);
                uint32_t hash47 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 44);
                uint32_t hash57 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 55);
                uint32_t hash67 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 66);
                uint32_t hash77 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 77);
                uint32_t hash87 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 88);
                __u32 pos17 = hash17 & (light_size_n - 1);
                __u32 pos27 = hash27 & (light_size_n - 1);
                __u32 pos37 = hash37 & (light_size_n - 1);
                __u32 pos47 = hash47 & (light_size_n - 1);
                __u32 pos57 = hash57 & (light_size_n - 1);
                __u32 pos67 = hash67 & (light_size_n - 1);
                __u32 pos77 = hash77 & (light_size_n - 1);
                __u32 pos87 = hash87 & (light_size_n - 1);
                ...
                __u32 *val37 = LightPart37.lookup(&hash37);
                if (val37)
                    (*val37) += 1;
                __u32 *val47 = LightPart47.lookup(&hash47);
                if (val47)
                    (*val47) += 1;
                __u32 *val57 = LightPart57.lookup(&hash57);
                if (val57)
                    (*val57) += 1;
                __u32 *val67 = LightPart67.lookup(&hash67);
                if (val67)
                    (*val67) += 1;
                __u32 *val77 = LightPart77.lookup(&hash77);
                if (val77)
                    (*val77) += 1;

                __u32 *val87 = LightPart87.lookup(&hash87);
                if (val87)
                    (*val87) += 1;
            }
    ...
    uint32_t hash6 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 1234);
    __u32 pos6 = hash6 & (heavy_size - 1);

    struct Bucket *hpb6;
    struct Bucket *hpb6_init;
    // hpb6_init->posvote = 1;
    // hpb6_init->negvote = 0;
    // hpb6_init->flag == false;
    // hpb6_init->flow_id.dstIP = dstIP;
    // hpb6_init->flow_id.srcIP = srcIP;
    __u32 *hpf6;
    hpf6 = HeavyParts_flag6.lookup(&pos6);
    hpb6 = HeavyParts_buckets6.lookup(&pos6);
    ...
    if (hpf6)
    {
        if (hpb6)
        {
            if (*hpf6 == 0)
            {
                // hpf6 = HeavyParts_flag6.lookup_or_try_init(&pos, &one);
                *hpf6 = 1;
                // hpb6 = HeavyParts_buckets6.lookup_or_try_init(&pos, &hpb6_init);
                hpb6->posvote = 1;
                hpb6->negvote = 0;
                hpb6->flag == false;
                hpb6->flow_id.dstIP = dstIP;
                hpb6->flow_id.srcIP = srcIP;
            }
            else if (*hpf6 != 0)
            {
                if (hpb6->flow_id.srcIP == srcIP && hpb6->flow_id.dstIP == dstIP)
                {
                    hpb6->posvote++;
                }
                else
                {
                    hpb6->negvote++;
                    u_int64_t temp6 = hpb6->negvote / hpb6->posvote;
                    if (temp6 < lamda)
                    {
                        res6 = 1;
                    }
                    else
                    {
                        ...
                        hpb6->posvote = 1;
                        uint32_t tmp6 = srcIP;
                        srcIP = hpb6->flow_id.srcIP;
                        hpb6->flow_id.srcIP = tmp6;
                        tmp6 = dstIP;
                        dstIP = hpb6->flow_id.dstIP;
                        hpb6->flow_id.dstIP = tmp6;
                    }
                }
            }
            ...
            if (res6 > 0)
            {
                ...
                uint32_t hash16 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 11);
                uint32_t hash26 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 22);
                uint32_t hash36 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 33);
                uint32_t hash46 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 44);
                uint32_t hash56 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 55);
                uint32_t hash66 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 66);
                uint32_t hash76 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 77);
                uint32_t hash86 = MurmurHash3_x86_32(&pkt, sizeof(pkt), 88);
                __u32 pos16 = hash16 & (light_size_n - 1);
                __u32 pos26 = hash26 & (light_size_n - 1);
                __u32 pos36 = hash36 & (light_size_n - 1);
                __u32 pos46 = hash46 & (light_size_n - 1);
                __u32 pos56 = hash56 & (light_size_n - 1);
                __u32 pos66 = hash66 & (light_size_n - 1);
                __u32 pos76 = hash76 & (light_size_n - 1);
                __u32 pos86 = hash86 & (light_size_n - 1);
                ...
                __u32 *val36 = LightPart36.lookup(&hash36);
                if (val36)
                    (*val36) += 1;
                __u32 *val46 = LightPart46.lookup(&hash46);
                if (val46)
                    (*val46) += 1;
                __u32 *val56 = LightPart56.lookup(&hash56);
                if (val56)
                    (*val56) += 1;
                __u32 *val66 = LightPart66.lookup(&hash66);
                if (val66)
                    (*val66) += 1;
                __u32 *val76 = LightPart76.lookup(&hash76);
                if (val76)
                    (*val76) += 1;

                __u32 *val86 = LightPart86.lookup(&hash86);
                if (val86)
                    (*val86) += 1;
            }
