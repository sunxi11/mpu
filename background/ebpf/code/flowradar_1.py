...
def get_cycles():
    ...
    outputfn="cycles/flowradar_1.log"
    ...
    for i in str_1:
        if i!='' and i[0]>='0' and i[0]<='9':
            num.append(int(i))
    outputfn=f'cycles/exp-cycles-7.log'
...
if __name__ == '__main__':
    ...
    parser.add_argument("-p", "--probability", required=False, type=float,default=0.3 ,help="The update probability of the sketch")
    ...
    custom_cflags.append(f"-I{sys.path[0]}/ebpf/nitrosketch_1")
    custom_cflags.append("-I/usr/include/linux")
    #32bit max value * probality
    update_probability = np.uint32((np.iinfo(np.uint32).max * probability))


    custom_cflags.append(f"-D_CT_K={ct_k}")
    custom_cflags.append(f"-D_W_CT={w_ct}")
    custom_cflags.append(f"-D_W_BF={w_bf}")
    custom_cflags.append(f"-D_BF_K={bf_k}")
    # custom_cflags.append(f"-D_NM_LAYERS={layers}")
    # custom_cflags.append(f"-D_HEAP_SIZE={MAX_HEAP_ENTRIES}")
    ...
    b = BPF(src_file=f"{sys.path[0]}/ebpf/new/bloomfilter/flowradar_1.h", cflags=custom_cflags, device=None)
    # load BPF program
   # b = BPF(src_file=f'{sys.path[0]}/ebpf/new/bloomfilter/flowradar_2.h', cflags=custom_cflags, device=None)
