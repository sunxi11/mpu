...
BPF_ARRAY(bloomfilters8, __u32, 262144);
BPF_ARRAY(countingtables8, struct FRBucket, 589824);
...
int xdp_prog1(struct CTXTYPE *ctx)
{
 ...
 bool res5 = true;
    bool res8 = true;
    __u64 len8 = sizeof(pkt);
    for (unsigned i = 0; i < bf_k; i++)
    {
        uint32_t hash8 = MurmurHash3_x86_32(&pkt, len8, i * i);
        __u32 target_idx8 = hash8 & (w_bf - 1);
        __u32 *val8 = bloomfilters8.lookup(&target_idx8);
        if (val8)
        { // 如果val8不是null
            if (*val8 == 0)
            {
    ...
    uint64_t k18, k28;
    k18 = (uint64_t)pkt.src_ip;
    k28 = (uint64_t)pkt.dst_ip;
    uint64_t combined_k8 = (k18 << 32) + k28;

    for (unsigned i = 0; i < ct_k; i++)
    {
        uint32_t hash8 = MurmurHash3_x86_32(&pkt, len8, i * i);
        __u32 target_idx8 = hash8 & (w_ct - 1);
        struct FRBucket *val8 = countingtables8.lookup(&target_idx8);
        if (val8)
        { // 如果val8不是null
            if (res8 == false)
            {
                val8->FlowXOR ^= combined_k8;
                val8->FlowCount++;
            }
            val8->PacketCount++;
        }
