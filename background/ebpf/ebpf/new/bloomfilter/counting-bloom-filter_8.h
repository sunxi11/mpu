...
BPF_ARRAY(countingbloomfilter6, __u32, 2621440);
BPF_ARRAY(countingbloomfilter7, __u32, 2621440);
BPF_ARRAY(countingbloomfilter8, __u32, 2621440);
...
#pragma unroll
    ...
    for (int i = 0; i < k; i++)
    {
        uint32_t hash8 = MurmurHash3_x86_32(&pkt, sizeof(pkt), i * i);
        __u32 target_idx8 = hash8 & (COLUMNS - 1);
        if (CHECK_BIT(hash8, 31))
        {
            __u32 *val8 = countingbloomfilter8.lookup(&target_idx8);
            if (val8)
            {
                (*val8) += 1;
            }
    ...
    for (int i = 0; i < k; i++)
    {
        uint32_t hash7 = MurmurHash3_x86_32(&pkt, sizeof(pkt), i * i);
        __u32 target_idx7 = hash7 & (COLUMNS - 1);
        if (CHECK_BIT(hash7, 31))
        {
            __u32 *val7 = countingbloomfilter7.lookup(&target_idx7);
            if (val7)
            {
                (*val7) += 1;
            }
    ...
    for (int i = 0; i < k; i++)
    {
        uint32_t hash6 = MurmurHash3_x86_32(&pkt, sizeof(pkt), i * i);
        __u32 target_idx6 = hash6 & (COLUMNS - 1);
        if (CHECK_BIT(hash6, 31))
        {
            __u32 *val6 = countingbloomfilter6.lookup(&target_idx6);
            if (val6)
            {
                (*val6) += 1;
            }
